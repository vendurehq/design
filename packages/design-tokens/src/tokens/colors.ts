export const brand = {
  50: 'oklch(0.97 0.02 231)',
  100: 'oklch(0.93 0.04 231)',
  200: 'oklch(0.87 0.07 231)',
  300: 'oklch(0.78 0.11 231)',
  400: 'oklch(0.68 0.15 231)',
  500: 'oklch(0.58 0.18 231)',
  600: 'oklch(0.50 0.16 231)',
  700: 'oklch(0.42 0.14 231)',
  800: 'oklch(0.34 0.11 231)',
  900: 'oklch(0.27 0.08 231)',
  950: 'oklch(0.20 0.06 231)',
} as const;

export const neutral = {
  50: 'oklch(0.97 0.005 231)',
  100: 'oklch(0.93 0.005 231)',
  200: 'oklch(0.87 0.005 231)',
  300: 'oklch(0.78 0.005 231)',
  400: 'oklch(0.68 0.01 231)',
  500: 'oklch(0.55 0.01 231)',
  600: 'oklch(0.45 0.01 231)',
  700: 'oklch(0.37 0.01 231)',
  800: 'oklch(0.28 0.01 231)',
  900: 'oklch(0.20 0.01 231)',
  950: 'oklch(0.15 0.01 231)',
} as const;

export const success = {
  50: 'oklch(0.97 0.02 145)',
  100: 'oklch(0.93 0.04 145)',
  200: 'oklch(0.87 0.07 145)',
  300: 'oklch(0.78 0.11 145)',
  400: 'oklch(0.68 0.15 145)',
  500: 'oklch(0.55 0.17 145)',
  600: 'oklch(0.48 0.15 145)',
  700: 'oklch(0.40 0.13 145)',
  800: 'oklch(0.33 0.10 145)',
  900: 'oklch(0.27 0.08 145)',
  950: 'oklch(0.20 0.06 145)',
} as const;

export const warning = {
  50: 'oklch(0.97 0.02 85)',
  100: 'oklch(0.93 0.05 85)',
  200: 'oklch(0.87 0.08 85)',
  300: 'oklch(0.78 0.12 85)',
  400: 'oklch(0.72 0.15 85)',
  500: 'oklch(0.65 0.15 85)',
  600: 'oklch(0.55 0.13 85)',
  700: 'oklch(0.45 0.11 85)',
  800: 'oklch(0.37 0.09 85)',
  900: 'oklch(0.28 0.06 85)',
  950: 'oklch(0.20 0.04 85)',
} as const;

export const destructive = {
  50: 'oklch(0.97 0.02 25)',
  100: 'oklch(0.93 0.05 25)',
  200: 'oklch(0.87 0.08 25)',
  300: 'oklch(0.78 0.12 25)',
  400: 'oklch(0.68 0.16 25)',
  500: 'oklch(0.55 0.20 25)',
  600: 'oklch(0.48 0.18 25)',
  700: 'oklch(0.40 0.15 25)',
  800: 'oklch(0.33 0.12 25)',
  900: 'oklch(0.27 0.09 25)',
  950: 'oklch(0.20 0.06 25)',
} as const;
