...
A `#` followed by a purely decimal 3- or 4-digit run (for example a GitHub issue reference like `(#2608)` in a test title) is not treated as a `#RGB`/`#RGBA` color, so such references are not flagged. Short hex colors that contain at least one `a`-`f` digit (`#abc`, `#f00`) are still rejected, as are 6- and 8-digit hex colors even when fully decimal (`#112233`).
